...
public sealed partial class CommandAppTests
{
    public sealed class FlagValues
    {
        ...
        internal sealed class FlagSettingsWithDefaultValue : CommandSettings
        {
            [CommandOption("--serve [PORT]")]
            [DefaultValue(987)]
            public FlagValue<int> Serve { get; set; } = null!;
        }
        ...
        public void Should_Set_Flag_And_Value_If_Both_Were_Provided()
        {
            ...
            // When
            var result = app.Run("foo", "--serve", "123");

            // Then
            result.ExitCode.ShouldBe(0);
            result.Settings.ShouldBeOfType<FlagSettings>().And(settings =>
            {
                settings.Serve.ShouldNotBeNull();
        ...
        public void Should_Create_Unset_Instance_With_Null_For_Nullable_Value_Type_If_Flag_Was_Not_Set()
        {
            ...
            // Then
            result.ExitCode.ShouldBe(0);
            result.Settings.ShouldBeOfType<FlagSettingsWithNullableValueType>().And(settings =>
            {
                settings.Serve.ShouldNotBeNull();
                settings.Serve.IsSet.ShouldBeFalse();
